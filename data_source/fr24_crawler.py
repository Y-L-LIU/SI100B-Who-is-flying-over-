...
class Fr24Crawler:
    ...
    def spin(self, loc: Tuple[float, float], rng, enabled, interval):
        while True:
            ct = time.time()
            self.update_settings(loc, rng, interval)
            self.get_data_once()
            c = self.plotter.update_settings(loc, rng, enabled, interval)
            if c:
...
if __name__ == '__main__':
    crawler = Fr24Crawler()
    crawler.update_settings((31.17940, 121.59043), (32.67940, 120.09043), 10)
    crawler.spin((31.17940, 121.59043), (32.67940, 120.09043), 10)
